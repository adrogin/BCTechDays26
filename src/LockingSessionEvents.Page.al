page 50700 "Locking Session Events"
{
    ...
    layout
    {
        area(Content)
        {
            repeater(Events)
            {
                field(EventTimeControl; EventTimeFormatted)
                {
                    ApplicationArea = All;
                    Caption = 'Event Time';
    ...
    begin
        EventTimeFormatted := Format(DT2Time(Rec."Event DateTime"), 0, '<Hours24>.<Minutes,2>.<Seconds,2><Second dec.>');
    end;
